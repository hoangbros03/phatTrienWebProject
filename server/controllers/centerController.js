...
const createNewCenter = async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ message: "user, pass, name, and region are required" });
  }
  ...
  if (body.user == "god") {
    ...
    return res
      .status(400)
      .json({ status: "Sorry, but this user name is prohibited" });
  }
  ...
  const testExistRegion = await region
    .findOne({ regionName: body.regionName })
    .exec();
  if (!testExistRegion) {
    logger.info(
      "regionName doesn't contain in the list, please check the spelling: " +
        body.regionName
    );
    return res
      .status(400)
  ...
  try {
    ...
    res
      .status(201)
      .json({
        success: `New center ${body["name"]} created!`,
        username: body["user"],
      });
  } catch (err) {
...
const uploadCenters = async (req, res) => {
  ...
  for (let e = 0; e < arr.length; e++) {
    ...
    try {
      let [jsonReturn, statusCode] = await vitalFunc.result(
        arr[e],
        "/cucDangKiem/" + "god" + "/center",
        "POST",
  ...
  res.json({ total: arr.length, uploaded: uploaded });
...
const changePasswordCenter = async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({
        status: "newPassword or old password not a string or doesn't exist!",
      });
  }
  var bypass = false;
  if (req?.body?.bypass) {
...
const addRegistry = async (req, res) => {
  ...
  if (
    ...
    typeof req.body.trungTamDangKiemName != "string"
  ) {
  ...
  if (
    isNaN(dateOfExpiry) ||
    isNaN(dateOfIssue) ||
    !dateOfExpiry instanceof Date ||
    !dateOfIssue instanceof Date
  ) {
    logger.info(
      "Either dateOfIssue or dateOfExpiry is not a valid ISO date format"
    );
    return res
      .status(400)
      .json({
  ...
  //check car valid
  var getCar = await Cars.findOne({ licensePlate: req.body.licensePlate })
    .populate({
      path: "carOwner",
      select: "name",
    })
  ...
  if (!getCar) {
    let err =
      "Can't find car with given license plate. Please note that the licensePlate must be correct";
    logger.info(err);
    return res
      .status(400)
      .json({ status: "Không tìm thấy biển số xe. Vui lòng nhập lại" });
  }
  console.log(getCar);
  //do'
  const ttdk = await TrungTamDangKiem.findOne({
    name: req.body.trungTamDangKiemName,
  }).exec();
  ...
  await newRegistryInfo
    .save()
    .then((doc) => {
      logger.info(
        "Successfully create new registration information the license plate: " +
          req.body.licensePlate
      );
    })
    .catch((err) => {
...
const getCenters = async (req, res) => {
  ...
  const centers = await TrungTamDangKiem.find({
...
const getListCenters = async (req, res) => {
  const result = await TrungTamDangKiem.find().exec();
  for (let i = 0; i < result.length; i++) {
    const { name, user, regionName,active } = result[i];
    result[i] = { name, user,regionName,key:i.toString(),active };
  }
  return res.status(200).json(JSON.stringify(result));
};
