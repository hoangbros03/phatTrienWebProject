...
const {
  model: carSpecs,
  carSpecsSchema,
} = require("../models/CarSpecification");
const {
  model: paperOfRecognition,
  paperOfRecognitionSchema,
} = require("../models/PaperOfRecognition");
...
} = require("../models/RegistrationInformation");
...
const DEFAULT_PASSWORD_TTDK_CREATION =
  process.env.DEFAULT_PASSWORD_TTDK_CREATION || "123456";
...
const getCarsList = async (req, res) => {
  if (!enoughInformationToGetList(req)) {
    return res
      .status(400)
      .json({ message: "Please give back-end enough information" });
  }
  //valid
  if (
    typeof req.body.month != "string" ||
    typeof req.body.province != "string" ||
    typeof req.body.quarter != "string" ||
    typeof req.body.ttdk != "string" ||
    typeof req.body.type != "string" ||
    typeof req.body.year != "string" ||
    typeof req.body.carType != "string"
  ) {
    logger.info("Either one of the input infor is not a string");
    return res
      .status(400)
      .json({ status: "Either one of the input infor is not a string" });
  }
  if (
    typeof Number.parseInt(req.body.month) != "number" ||
    ...
    return res
      .status(400)
      .json({ status: "month or quarter or year is not a number" });
  }
  ...
  if (req.body.type == "registered" || req.body.type == "đã đăng kiểm") {
    cars = await registrationInformation
      .find({
        $expr: {
          $and: [
            {
    ...
    req.body.type === "nearexpire" ||
    req.body.type === "sắp đến hạn"
  ) {
      ...
      .find({
        ...
        $expr: {
          $and: [
            ...
            {
              $lte: ["$dateOfExpiry", expireDate],
            },
            {
              $gte: ["$dateOfExpiry", curDate],
            },
          ],
        },
      })
...
const searchCar = async (req, res) => {
  ...
  if (!car) {
    ...
    try {
      ...
      const regisInfor = await registrationInformation
        .find({
          licensePlate: lp,
        })
        .exec();
      ...
      logger.info(
        "err when get registration informations in the history:" + err
      );
      return res.status(400).json({
        status: "err when get registration informations in the history:",
      });
    }
  }
...
const createCar = async (req, res) => {
  ...
  if (
    ...
    !req?.body?.engineNo ||
    !req?.body?.classisNo ||
    !req?.body?.trungTamDangKiemName ||
    !req?.body?.trungTamDangKiemName
  ) {
    logger.info("Not enough information to create a car");
    return res
      .status(400)
      .json({ status: "Không đủ thông tin để đăng kiểm lần đầu" });
  }
  ...
  if (req.body.organization != true && req.body.organization != false) {
    ...
    return res
      .status(400)
      .json({ status: "Phải xác định là xe công vụ hay không" });
  }
  //2. ownerName
  //correctness
  ...
  if (
    ...
    req.body.ownerName.length < 5 ||
    !req.body.ownerName.match(vitalFunc.regexMatchVietNamese)
  ) {
    logger.info(
      "ownerName must be a string and have a proper length (full name) and space between first name and last name!"
    );

    return res.status(400).json({ status: "Tên chủ sở hữu chưa hợp lê" });
  }
  //3. licensePlate
  //correctness
  ...
  if (
    !req.body.licensePlate.match(/\d{2}[A-Z]-\d{3}[.]\d{2}$/) &&
    !req.body.licensePlate.match(/\d{2}[A-Z]-\d{4}$/)
  ) {
  ...
  if (isNaN(regionNumber)) {
    ...
    logger.info(
      "the licensePlate is either not a string or not a proper syntax. Please check again"
    );
  ...
  //check region
  //correctness
  req.body.regionName = vitalFunc.toTitleCase(
    req.body.regionName.toLowerCase()
  );
  ...
  if (!checkRegion) {
    ...
    return res
      .status(400)
      .json({ status: "Biển số xe và tỉnh không đúng quy định" });
  }
  ...
  if (typeof req.body.carType != "string") {
    ...
    return res
      .status(400)
      .json({ status: "Không có loại xe đấy hoặc chưa đúng loại xe" });
  }
  req.body.carType = req.body.carType.toLowerCase();
  ...
  if (
    typeof req.body.engineNo != "string" ||
    typeof req.body.classisNo != "string"
  ) {
    logger.info("car engine or classis no is not a string");
    return res
      .status(400)
      .json({ status: "Số máy và số khung chưa đúng quy định" });
  }
  //10. TTDK name
  ...
  // find ttdk
  let ttdk_found = await trungTamDangKiem
    .findOne({ name: req.body.trungTamDangKiemName })
    .exec();
  if (!ttdk_found) {
  ...
  if (typeof req.body.trungTamDangKiemName != "string") {
    ...
    return res
      .status(400)
      .json({ status: "Trung tam dang kiem name is not a string" });
  }
  // expire time
  ...
  if (req?.body?.expire) {
    if (isNaN(Number(req.body.expire))) {
      ...
      return res
        .status(400)
        .json({ status: "expire provided, but not a number" });
    }
  ...
  let carSpecCheck = await carSpecs
    .findOne({
      name: req.body.carName,
      version: req.body.carVersion,
      type: req.body.carType,
    })
    .exec();
  if (!carSpecCheck) {
    logger.info(
      "car specs isn't existed in db. Please re-check your information"
    );
  ...
  carSpecification = await carSpecs
    .findOne({
      name: req.body.carName,
      version: req.body.carVersion,
      type: req.body.carType,
  ...
  if (!carSpecification) {
    logger.info(
      "this car info isn't existed. Re-check the information. Remember that carSpec must be CASE-SENSITIVE. CONVERT TO CORRECT CASE NOT SUPPORTED"
    );
    return res
      .status(400)
      .json({ status: "Thông tin xe chưa đúng vui lòng kiểm tra lại" });
  }
  ...
  await newPaper
    ...
    .catch((err) => {
      logger.info(
        "There is an error when creating new document to save to the model: " +
          err
      );
      forceStop = true;
      return res.status(400).json({ status: "Có lỗi khi tạo xe " });
  ...
  try {
    ...
    const result = await newCar
      ...
      .catch((err) => {
        logger.info(
          "All information valid. Potential bug when creating car?: " + err
        );
        return res.status(400).json({ status: "Có lỗi khi tạo xe " });
      });
  } catch (err) {
...
const createCarSpecification = async (req, res) => {
  ...
  if (!req?.body?.name || !req?.body?.version || !req?.body?.type) {
    ...
    return res
      .status(400)
      .json({ status: "Not enough information to create new car spec" });
  }
  //check valid
  //correctness
  ...
  if (!carTypes.includes(req.body.type)) {
    ...
    return res
      .status(400)
      .json({ status: "Car type not existed. Check your spelling" });
  }
  //create and notify
  const newCarSpec = new carSpecs({
    ...
    numberOfSeats: checkNoRequireInformation(req.body.numberOfSeats, "number"),
  ...
  await newCarSpec
    ...
    .catch((err) => {
      ...
      return res
        .status(400)
        .json({ status: "Something wrong when creating newCarSpec: " });
    });
};
...
const uploadDB = async (req, res) => {
  ...
  try {
    ...
    if (!req?.body?.status) {
      ...
      return res
        .status(400)
        .json({ status: "wrong syntax when sending request!" });
    }
    ...
    for (let i = 0; i < req.body.status.length; i++) {
      ...
      try {
        var carSpecExisted = await carSpecs
          .findOne(currentCar.carSpecification)
          .exec();
        //create car spec
        if (!carSpecExisted) {
          var [jsonObj, statusCode] = await vitalFunc.result(
            currentCar.carSpecification,
            "/trungTamDangKiem/god/carSpec",
            "POST",
        ...
        if (statusCode != 200) {
          ...
          logger.info("Car spec  added success...");
        }
      } catch (err) {
        logger.info("Err: " + err);
      }
      ...
      //check ttdk
      let TTDKexisted = await trungTamDangKiem
        .findOne({
          name: currentCar.registrationInformation.trungTamDangKiem.name,
        })
        .exec();

      if (!TTDKexisted) {
        missRegistryRegistration++;
        continue;
      }
      //check car existed
      let carFound = await Cars.findOne({
        ...
        [jsonObj, statusCode] = await vitalFunc.result(
          {
            ...
            licensePlate: currentCar.licensePlate,
            dateOfIssue: currentCar.registrationInformation.dateOfIssue,
            regionName: currentCar.regionName,
            carName: currentCar.carSpecification.name,
            carVersion: currentCar.carSpecification.version,
            ...
            address: checkNoRequireInformation(
              currentCar.carOwner.address,
              "string"
            ),
            trungTamDangKiemName:
              currentCar.registrationInformation.trungTamDangKiem.name,
          },
          "/trungTamDangKiem/god/createCar",
          "POST",
          req?.headers?.authorization,
          false,
          true
      ...
      for (
        let j = 0;
        j < currentCar.historyRegistrationInformation.length;
        j++
      ) {
        historyRegister = currentCar.historyRegistrationInformation[j];
        //check required field inside
        if (
          !historyRegister.regionName ||
          !historyRegister.licensePlate ||
          !historyRegister.dateOfIssue ||
          !historyRegister.dateOfExpiry ||
          !historyRegister.ownerName ||
          !historyRegister.carType ||
          !historyRegister.trungTamDangKiemName
        ) {
        ...
        //check valid
        if (
          typeof historyRegister.regionName != "string" ||
          typeof historyRegister.licensePlate != "string" ||
          typeof historyRegister.dateOfIssue != "string" ||
          typeof historyRegister.dateOfExpiry != "string" ||
          typeof historyRegister.ownerName != "string" ||
          typeof historyRegister.carType != "string" ||
          typeof historyRegister.trungTamDangKiemName != "string"
        ) {
          logger.info(
            "Info inside history reg info not a string. Continuting..."
          );
          missRegistryRegistration += 1;
          continue;
        }
        //add new history information, save id for further purpose with newest registry
          ...
          var [registryJSON, registryStatusCode] = await vitalFunc.result(
            {
              licensePlate: historyRegister.licensePlate,
              dateOfIssue: historyRegister.dateOfIssue,
              dateOfExpiry: historyRegister.dateOfExpiry,
              trungTamDangKiemName: historyRegister.trungTamDangKiemName
            },
            "/trungTamDangKiem/god/newRegistry",
            "POST",
            req?.headers?.authorization,
            false,
            true
        ...
        if (registryStatusCode == "200") {
          ...
          if (
            ...
            //correctness
            historyRegister.regionName =
              vitalFunc.toTitleCase(
                currentCar.historyRegistrationInformation[
                  j
                ].regionName.toLowerCase()
              );
            let regis = await registrationInformation
              .findOne({
                licensePlate:
                  historyRegister.licensePlate,
                dateOfIssue:
          ...
          logger.info(
            "There is an error when creating registry number " + (j + 1) + "."
          );
          missRegistryRegistration += 1;
          continue
        }
    ...
    res.json({
      totalCar: req.body.status.length,
...
const exportCars = async (req, res) => {
  ...
  let jsonData = await vitalFunc.result(
    req.body,
    "/trungTamDangKiem/god/carList",
    "POST",
    req?.headers?.authorization,
    false
  );
  ...
  for (let i = 0; i < jsonData.length; i++) {
    try {
      ...
      logger.info(
        "Error when appending licensePlate: " + err + ". Process continuting..."
      );
    }
  }
  ...
  for (let i = 0; i < licensePlateList.length; i++) {
    let car = await vitalFunc.result(
      { searchValue: licensePlateList[i] },
      "/trungTamDangKiem/god/searchCar",
      "POST",
      req?.headers?.authorization,
      false
    );
    ...
    if (car == "No car match") {
      logger.info(
        "Registration information found but no car found. This car may be removed. Process continuting..."
      );
      continue;
    }
    if (car._id) {
    ...
    if (
      ...
      car.registrationInformation.trungTamDangKiem &&
      car.registrationInformation.trungTamDangKiem._id
    ) {
...
const deleteCar = async (req, res) => {
  ...
  if (
    ...
    logger.info(
      "licensePlate when deleting must be exactly the same. The information inputted isn't match the regex"
    );
    return res.status(400).json({
      status:
        "licensePlate when deleting must be exactly the same. The information inputted isn't match the regex",
  ...
  try {
    await paperOfRecognition
      .deleteOne({ licensePlate: req.body.licensePlate })
      .then(() => {
        logger.info("Delete successfully!");
      })
      .catch((err) => {
        logger.info("An error when deleting paperOfRecognition:" + err);
      });
    await Cars.deleteOne({ licensePlate: req.body.licensePlate })
      .then(() => {
        logger.info("Delete successfully!");
      })
      .catch((err) => {
    ...
    await registrationInformation
      .deleteMany({ licensePlate: req.body.licensePlate })
      .then(() => {
        logger.info("Delete all registration information ok");
      })
      .catch((err) => {
        logger.info("err: " + err);
      });
